...
- Definir independientemente los parámetros de trabajo para cada objeto en el panel de propiedades, "Propiedades CAM". Cuando se utilizan los valores particulares de objeto, Cada pasada bajará el valor de "StepDown" de las "Propiedades CAM". En estas propiedades se pueden definir dos modos de trabajo; "Perfil" para trayectorias lineales y "Pocket" para superficies cerradas. en el caso de seleccionar "Pocket" se habilita la entrada de Paso lateral que es la que define la distancia horizontal entre cada pasada. Tambien se puede definir un angulo de pendiente para la herramienta. El modo Pocket solo funciona con la figura "Rectángulo".
- Para el trazado de figuras y piñones de cadena, despues de definir las dimensiones del objeto, hacer un offset exterior a la medida del radio de la herramienta para que la dimension final sea la deseada.
- Entre las propiedades de material en bruto también se configura una altura de seguridad de pasada. Esta altura se suma al espesor del material y será la altura que cogerá la herramienta en su inicio y la que cogerá en los movimientos rápidos entre figuras.
